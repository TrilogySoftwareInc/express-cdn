      ...
      case 'uglify':
        ...
        try {
          ...
          if (cacheDir) {
            ...
            logger({ task: 'express-cdn', fileName, message: `Wrote to ${cacheFile}` });
          }
        } catch (er) {
          logger({ task: 'express-cdn', fileName, message: er });
        }
        ...
        if (uglifyResult.error !== undefined) {
          logger({
            ...
            console.log(`[${fileName}]`, uglifyResult);
          if (options.allowUnminified === false) {
        ...
        logger({
          task: 'express-cdn',
          fileName,
          message: 'Uploading to S3 anyway',
        });
        pushFileToS3(S3, options, uglifyResult.code, fileName, headers, callback);
        break;
      case 'minify':
        if (!(results instanceof Array)) {
          results = [results];
...
var checkArrayIfModified = function(assets, fileName, S3, options, timestamp, type, callback) {
  ...
  return function(/** @type {import('aws-sdk').AWSError} */err, /** @type {import('aws-sdk').S3.HeadObjectOutput} */ response) {
    if (response && timestamp <= response.LastModified) {
      logger({
        task: 'express-cdn',
...
var processAssets = function(options, results, done) {
  ...
  async.map(results, function(result, iter) {
    ...
    if (assets instanceof Array) {
      ...
      S3.headObject({ Bucket: options.bucket, Key: fileName }, checkArrayIfModified(assets, fileName, S3, options, timestamp, type, iter));
    } else {
      // Set the file name
      ...
      fs.exists(assets, function(exists) {
        ...
        S3.headObject({ Bucket: options.bucket, Key: fileName }, checkStringIfModified(assets, fileName, S3, options, timestamp, iter));
      });
    }
